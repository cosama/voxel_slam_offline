...
void pvec_update(PVecPtr pptr, IMUST &x_curr, PLV(3) &pwld)
{
  ...
  for(pointVar &pv: *pptr)
  {
    ...
    // `cov.block<3,3>(0,0)` is a body-frame rotation covariance (the state uses
