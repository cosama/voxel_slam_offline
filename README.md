...
The optional runner owns dataset plumbing: bag reading, URDF extrinsics, JSON
config loading, trajectory CSV writing, and optional PLY export.

```bash
python -m voxelslam.run_bag BAG URDF config.json output_dir
# or, when installed with the runner extra:
voxelslam-run-bag BAG URDF config.json output_dir
```

Install runner dependencies with:

```bash
pip install 'voxel-slam-offline[runner]'
```

The runner config is flat JSON. Voxel-SLAM fields use the same names as
`VoxelSlamConfig`; runner-only keys are `imu_topic`, `points_topic`,
`imu_frame`, `lidar_frame`, `scan_duration`, `stamp_is_end`, `ply`,
`max_lidar_messages`, `progress_interval`, `finish_poll_seconds`, and
`finish_timeout_seconds`.
`ply` can be `none`, `map`, or `dense`. Dense PLY output spools local deskewed
scans during the run, then applies the final trajectory when `finish()` returns.
PLY vertices include `timestamp`, `x`, `y`, `z`, and `intensity`.
Each run writes `trajectory.csv`, `manifest.json`, and `map.ply` when PLY output
is enabled.
